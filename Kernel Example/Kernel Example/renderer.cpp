...
namespace render {
	...
	bool init() {
		gdi_select_brush = (gdi_select_brush_t)get_system_module_export("\\SystemRoot\\System32\\win32kfull.sys", "NtGdiSelectBrush");
		if (!gdi_select_brush) {
			print("failed to get gdi_select_brush\n");
			return false;
		}

		nt_gdi_pal_blt = (pal_blt_t)get_system_module_export("\\SystemRoot\\System32\\win32kfull.sys", "NtGdiPatBlt");
		if (!nt_gdi_pal_blt) {
			print("failed to get nt_gdi_pal_blt\n");
			return false;
		}

		nt_user_get_dc = (nt_user_get_dc_t)get_system_module_export("\\SystemRoot\\System32\\win32kbase.sys", "NtUserGetDC");
		if (!nt_user_get_dc) {
			print("failed to get nt_user_get_dc\n");
			return false;
		}

		nt_gdi_create_solid_brush = (nt_gdi_create_solid_brush_t)get_system_module_export("\\SystemRoot\\System32\\win32kfull.sys", "NtGdiCreateSolidBrush");
		if (!nt_gdi_create_solid_brush) {
			print("failed to get nt_gdi_create_solid_brush\n");
			return false;
		}

		release_dc = (release_dc_t)get_system_module_export("\\SystemRoot\\System32\\win32kbase.sys", "NtUserReleaseDC");
		if (!release_dc) {
			print("failed to get release_dc\n");
			return false;
		}

		delete_object_app = (delete_object_app_t)get_system_module_export("\\SystemRoot\\System32\\win32kbase.sys", "NtGdiDeleteObjectApp");
